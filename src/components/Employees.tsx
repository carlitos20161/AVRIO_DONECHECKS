...
const Employees: React.FC<EmployeesProps> = ({ currentRole, companyIds }) => {
  ...
  const handleAdd = async () => {
    ...
    // Auto-create relationship for single employees
    let clientPayTypeRelationships = newEmployee.clientPayTypeRelationships || [];
    
    // If it's a single employee (not multiple clients) and has a clientId, create a relationship
    if (!newEmployee.hasMultipleClients && newEmployee.clientId && newEmployee.payType) {
      const relationshipId = Date.now().toString();
      const client = clients.find(c => c.id === newEmployee.clientId);
      clientPayTypeRelationships = [{
        id: relationshipId,
        clientId: newEmployee.clientId,
        clientName: client?.name || 'Unknown Client',
        payType: newEmployee.payType as "hourly" | "perdiem",
        payRate: (isNaN(parsedRate) ? 0 : parsedRate).toString(),
        active: true
      }];
    }
    ...
    const data: any = {
      ...
      clientPayTypeRelationships: clientPayTypeRelationships,
    ...
    if (selectedCompanyId) {
      ...
      // Auto-assign to first available company if none selected
